...
\begin{align*}
  ...
  \mathrm{Esc}_t&:=\mathbf 1\{\text{round $t$ is escalated}\}.
\end{align*}
...
\(I(\theta;X)\le I\) under the \emph{uniform} prior on an $M$-point
label set ($M\ge2$).  Then
\begin{equation}\label{eq:ErrLower-new}
  \boxed{\;
    \Err_{\mathrm{bar}}
      \;\ge\;
      \frac{\delta\,T}{2}\;
      \Bigl(\bar\kappa+\gamma-1\Bigr),
      \quad
      \gamma:=\frac12\Bigl(1-\tfrac{I+\log 2}{\log M}\Bigr)
  }.
\end{equation}
In particular, the bound is positive—and linear in $T$—whenever the
validator escalates fewer than a $(1-\gamma)$-fraction of rounds
\((\bar\kappa>1-\gamma)\).
\end{lemma}
...
\(L^\star(m,x):=\mathbf 1\!\bigl[\Err_m(x)>
   \tfrac12\delta\kappa(\omega)T\bigr]\)
and $\Delta=\tfrac12$ in Proposition 3 of
\citet{chen2024interfano}.
Inequality ($\ast$) fulfils the separation requirement
\citep[Eq.\,(5)]{chen2024interfano}; thus
\[
  \Pr\!\bigl[\Err(\theta,X)>
     \tfrac12\delta\kappa(\omega)T\bigr]
  \;\ge\;\gamma.
...
\]
apply the basic fact $\E[Z]\ge a\,\Pr(Z>a)$ to
\(Z=\Err\) and \(a=\tfrac12\delta\,\kappa T\):
\[
\begin{aligned}
  \Err_{\mathrm{bar}}
    &=\E[\Err] \\[2pt]
    &\ge
      \E\!\Bigl[
        ...
        \mathbf 1_{\{\Err>\tfrac12\delta\,\kappa T\}}
      \Bigr] \\[6pt]
    &=\frac{\delta T}{2}\,
      \E\!\bigl[\kappa\,\mathbf 1_{\mathcal E}\bigr] \\[6pt]
    &=\frac{\delta T}{2}\,
      \kappa_{\!*}\;
      \Pr(\mathcal E).
\end{aligned}
...
Write the bound as  
\(\Err_{\mathrm{bar}}\!\ge(\delta T/2)\,\max\{0,\bar\kappa+\gamma-1\}\).
Because \(\gamma>0\) whenever the transcript leaks
fewer than \(\log M-\log2\) bits, any validator that
handles a \emph{non-trivial} share of rounds automatically
\((\bar\kappa>1-\gamma)\)
faces a \emph{linear-in-$T$} unavoidable error floor.
