...
\begin{equation}\label{eq:ErrDef}
  \Err(\omega):=\sum_{t\in\mathcal L(\omega)}
                \mathbf 1_{\!\bigl[h_t\neq y_t\bigr]}.
\end{equation}
For any hypothetical label index \(m\) and transcript \(x\) we write
\[
  \Err_m(x):=\sum_{t\in\mathcal L(\omega)}\mathbf 1_{\!\bigl[h_t(x)\neq y_t(m)\bigr]},
\]
i.e.\ the cumulative error that would occur on the same leverage rounds if the
hidden parameter were \(m\).
...
Step 1 (separation).
Fix \(m\neq m'\) and transcript \(x\).  On each leverage round
\(t\in\mathcal L(\omega)\) we have \(|y_t(\theta)-h_t|\ge\delta\).
Hence \(h_t\) can coincide with at most one of
\(y_t(m),y_t(m')\), so
\[
...
(In the worst‑case geometry one needs an extra factor \(2^{d}\); we omit it
because all subsequent results depend only on the \(\tau^{-d}\) scaling.)
\end{proof}
...
\begin{proof}[Sketch]
1.~Maximal \(3\tau\)-packing.\footnote{Using \(3\tau\) rather than \(\tau\) only
shrinks the packing constant by \(3^{-d}\); the \(\Theta(\tau^{-d})\) scaling
is unaffected.}
Let \(\{c_i\}_{i=1}^{m}\subset\mathcal Q\) be maximal with pairwise
distances \(>3\tau\).  Axis‑aligned cubes of side \(3\tau/\sqrt d\) give
\(m\ge\alpha_d(D/\tau)^d\) with \(\alpha_d=3^{-d}2^{-d}\).

\noindent
2.~Disjoint balls and guaranteed insertion.  
Set \(B_i=B(c_i,\tfrac{\tau}{2})\).  
For any query \(q_t\in B_i\) and any prior memory point \(m\in B_j\;(j\neq i)\),
\[
\lVert q_t-m\rVert
\;>\; 3\tau-\tfrac{\tau}{2}-\tfrac{\tau}{2}
\;=\;2\tau>\tau,
\]
so the insert-iff-distance \(>\tau\) rule is automatically satisfied.
The
balls are disjoint and lie inside \(\mathcal Q\).
...
\noindent
5.~Union bound and memory lower bound.  
With probability \(\ge 1-\delta\) every \(B_i\) is hit at least once.
If the first hit is accepted, it is written to memory; if it is rejected, the
blocking memory point \(m\) satisfies
\(\lVert m-c_i\rVert<1.5\,\tau\) and therefore lies in the expanded ball
\(\tilde{B}_i:=B(c_i,1.5\,\tau)\).
Because the \(\tilde{B}_i\) are disjoint (centre separation \(>3\tau\)),
each centre still accounts for one distinct stored point, so
\(|M_T|\ge m\ge\alpha_d(D/\tau)^d\).
\end{proof}
